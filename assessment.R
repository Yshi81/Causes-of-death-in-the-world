...
names(data_line) <- names(data_all[5:41])
data_line$Year <- c(1990:2019)
...
bar_chart <- ggplot(data = data_bar, mapping = aes(x = Year, y = Cardiovascular_diseases,fill = Age)) + 
  ...
  scale_fill_manual(name = 'Age',
                    values = c('#FF4500','#9400D3','#F4A460','#90EE90','#6495ED')) +
  theme_minimal() 
ggplotly(bar_chart)
...
value$RATIO <- value$VALUE / data_map$POP_EST
shape <- merge(data_map, value, by = "NAME_LONG") 
i_popup <- paste0("<strong>Country: </strong>", shape$NAME_LONG, "<br>", "<strong>Ratio: </strong>", shape$RATIO) 
pal <- colorBin(c("darkred", "orangered" ,"yellow", "darkgreen", "blue"), shape$RATIO, 11)
map <- leaflet(shape) %>% addTiles() %>% 
  addProviderTiles("Esri.WorldStreetMap") %>% 
  setView(0.000000, 0.000000, zoom = 2) %>% 
  addPolygons(color = ~pal(shape$RATIO), fillOpacity = 0.8, weight = 1, popup = i_popup) %>% 
  addLegend(pal = pal, values = shape$RATIO, position = "bottomright", title = "Data Map")
map
